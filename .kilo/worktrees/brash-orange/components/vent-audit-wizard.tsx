...
import { auditContext, auditProjectSchema, calculateAudit, inspectSystem, newAuditProject, newOpening, openingSize, wallLength, type AuditProject, type Opening } from "@/lib/vent-audit"
import { foundationInputSchema } from "@/lib/vent-calculator"
import { materialOptions } from "@/lib/site-data"

const STORAGE = "sunbur:vent-audit:v2"
...
export function VentAuditWizard({ children, onLegacy }: { children?: ReactNode; onLegacy: () => void }) {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) { const parsed = auditProjectSchema.safeParse(JSON.parse(saved)); if (parsed.success) { setProject(parsed.data); setNotice("Ваш дом восстановлен. Можно продолжить с того места, где остановились.") } }
  ...
  async function importFile(file?: File) {
    ...
    try {
      ...
      const parsed = auditProjectSchema.safeParse(raw)
      if (!parsed.success) throw new Error("Не удалось открыть проект. Выберите файл, сохранённый этим калькулятором.")
      setUndo(project); setProject(parsed.data); setOpeningId(null); setWallId("wall-1"); setStep(0); setNotice("Проект открыт. Все проверки пересчитаны.")
        ...
        <div className="va-work">
          <section className="va-section va-no-print">
            ...
            {step === 0 && <FieldGroup>
              ...
              <details className="va-details"><summary>Есть внутренние стены под полом? <span>{project.partitions.length ? `${project.partitions.length} добавлено` : "Добавить перемычки"}</span></summary><p>Добавьте сплошные стены, которые делят подполье на отсеки. Проходы в них отметим на следующем шаге. Если стену можно обойти сбоку, этот шаблон её не описывает.</p>
                {project.partitions.map((part, i) => <div className="va-partition" key={part.id}><strong>Перемычка {i + 1}</strong><SelectField label="Направление" value={part.axis} onChange={v => change({ ...project, partitions: project.partitions.map(a => a.id === part.id ? { ...a, axis: v as "x" | "z" } : a) })}><option value="x">Слева направо на плане</option><option value="z">Сверху вниз на плане</option></SelectField><NumberField label={part.axis === "x" ? "От верхней стороны плана" : "От левой стороны плана"} min={0.1} max={(part.axis === "x" ? project.foundation.width : project.foundation.length) - 0.1} value={part.position} onChange={v => change({ ...project, partitions: project.partitions.map(a => a.id === part.id ? { ...a, position: v } : a) })} /><div className="va-fields-grid"><SelectField label="Материал перемычки" value={part.material} onChange={v => change({ ...project, partitions: project.partitions.map(a => a.id === part.id ? { ...a, material: v as AuditProject["partitions"][number]["material"] } : a) })}>{materialOptions.map(m => <option key={m.value} value={m.value}>{m.label}</option>)}</SelectField><NumberField label="Толщина перемычки" unit="мм" value={part.thickness} min={100} max={1000} onChange={v => change({ ...project, partitions: project.partitions.map(a => a.id === part.id ? { ...a, thickness: v } : a) })} /></div><Button variant="ghost" onClick={() => { setUndo(project); change({ ...project, partitions: project.partitions.filter(a => a.id !== part.id) }, false, true) }}><Trash2 data-icon="inline-start" />Удалить перемычку {i + 1}</Button></div>)}
                <Button variant="outline" disabled={project.partitions.length >= 8} onClick={() => change({ ...project, partitions: [...project.partitions, { id: nextId("partition-", project.partitions.map(a => a.id)), axis: "x", position: Number((project.foundation.width / 2).toFixed(2)), material: "brick", thickness: 200 }] })}><Plus data-icon="inline-start" />Добавить внутреннюю стену</Button>
              </details>
