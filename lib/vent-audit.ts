...
const EPS = 1e-7
const MIN_PARTITION_LENGTH = 0.2
...
export const auditProjectSchema = z.object({
  version: z.literal(3), mode: z.literal("audit"),
  foundation: foundationInputSchema,
  kind: z.enum(["crawlspace", "basement", "slab", "unknown"]),
  partitions: z.array(z.object({ id, axis: z.enum(["x", "z"]), position: n(0.1, 39.9), from: n(0, 40), to: n(0, 40),
  ...
  p.partitions.forEach((part, i) => {
    if (part.to - part.from < MIN_PARTITION_LENGTH - EPS) ctx.addIssue({ code: "custom", path: ["partitions", i, "to"], message: `Длина перемычки должна быть не меньше ${MIN_PARTITION_LENGTH} м, а конец — дальше начала.` })
  })
})
...
export const newAuditProject = (): AuditProject => ({ version: 3, mode: "audit", foundation: { ...DEFAULT_FOUNDATION, partitions: "none" }, kind: "crawlspace", partitions: [], openings: [], blocked: [], groundLevel: null, allowNew: true, inventoryComplete: false })
...
export function auditGeometry(p: AuditProject) {
  ...
  for (const [index, part] of p.partitions.entries()) {
    const from = Math.min(part.from, part.to), to = Math.max(part.from, part.to)
    // Perimeter/notch corners plus the partition's own bounds — clips the line to
    // its requested length instead of always spanning the whole footprint.
    const coords = [...new Set([...geometry.vertices.map(v => part.axis === "x" ? v.x : v.z), from, to])]
      .filter(v => v >= from - EPS && v <= to + EPS).sort((a, b) => a - b)
...
export function inspectSystem(p: AuditProject, openings = p.openings, c = auditContext(p)) {
  ...
  for (const part of p.partitions) if (!c.geometry.walls.some(w => w.id.startsWith(`${part.id}:`))) issues.push({ code: "partition", level: "problem", text: "Одна из перемычек находится за пределами фундамента. Уточните её положение или длину." })
  const overlapping = p.partitions.some((a, i) => p.partitions.some((b, j) => j > i && a.axis === b.axis && Math.abs(a.position - b.position) < EPS &&
    Math.min(a.from, a.to) < Math.max(b.from, b.to) - EPS && Math.min(b.from, b.to) < Math.max(a.from, a.to) - EPS))
  if (overlapping) issues.push({ code: "partition", level: "problem", text: "Две перемычки на одной линии перекрываются по длине. Уточните положение или длину." })
...
/** Upgrades a v2 project (partitions always spanned the whole footprint) to v3
 *  (partitions have their own `from`/`to` length) by giving each old partition
 *  the full span it used to draw implicitly. Returns null for anything else unrecognised. */
export function migrateAuditProject(raw: unknown): AuditProject | null {
  const direct = auditProjectSchema.safeParse(raw)
  if (direct.success) return direct.data
  if (!raw || typeof raw !== "object") return null
  const obj = raw as Record<string, unknown>
  if (obj.version !== 2 || obj.mode !== "audit" || !obj.foundation || typeof obj.foundation !== "object") return null
  const foundation = obj.foundation as Record<string, unknown>
  const partitions = Array.isArray(obj.partitions) ? obj.partitions.map((part: unknown) => {
    if (!part || typeof part !== "object") return part
    const p = part as Record<string, unknown>
    const limit = p.axis === "x" ? foundation.length : p.axis === "z" ? foundation.width : undefined
    return { ...p, from: 0, to: typeof limit === "number" ? limit : 40 }
  }) : obj.partitions
  const migrated = auditProjectSchema.safeParse({ ...obj, version: 3, partitions })
